...
pub struct Matrix {
    ...
    pub devices: u8,
    pub buffer: Vec<u8> // with length of devices * 2
}
impl Matrix {
    pub fn new(
        cs: LineHandle,
        ...
        Matrix { cs, data, clock, devices, buffer: vec![0; devices as usize * 2] }
    }
    ...
    pub fn draw_grid(&mut self, grid: Grid) -> Result<(), Error> {
        /* matrices chained like this: 
             ####┐
            ┌####┘
            └####
        */
        if grid.width * grid.height <= self.devices as usize {
            for display in 0..grid.width * grid.height {//grid.width * grid.height {
                ...
                // odd rows need special rotation and x position shifting when wired like this: =]
                let odd_row = {(display / grid.width) % 2 == 1};
                // x position shifting
                if odd_row {
    ...
    fn write_raw(&mut self, addr: u8, header: u8, data: u8) -> Result<(), Error> {
        ...
        if (offset + 1) < max_bytes as usize {
